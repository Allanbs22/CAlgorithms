void agregarEntrys(GtkWidget *dummy, gpointer widget){
    ...
    if(strcmp(modo,"cargar") != 0){
        ...
        for(int i=0;i<otro;i++){
            ...
            sprintf(texto,"Cantidad de filas matriz %i (D%i):",i+1,i);
            if(i==otro-1)
            	sprintf(texto,"Cantidad de columnas matriz %i (D%i):",i,i);
            GtkWidget *lblPeso = gtk_label_new(texto);
            gtk_fixed_put(GTK_FIXED(fixInfo), lblPeso, 5, 40*i);


            GtkWidget *spinPeso = gtk_spin_button_new_with_range  (1, 9999, 1);
            gtk_fixed_put(GTK_FIXED(fixInfo), spinPeso, 260, 40*i);
        ...
        for(int i=0;i<otro;i++){
            ...
            GtkWidget *spinPeso = gtk_spin_button_new_with_range  (1, 9999, 1);
            gtk_spin_button_set_value (GTK_SPIN_BUTTON(spinPeso),lista[i]);
            gtk_fixed_put(GTK_FIXED(fixInfo), spinPeso, 260, 40*i);
...
void enviarAVentanaResultados(GtkWidget *dummy, gpointer widget){
    if(cantE>0){
        ...
        int elementos[cantE+1];
        for(int i=0;i<cantE+1;i++){
            elementos[i] = gtk_spin_button_get_value (GTK_SPIN_BUTTON(pesos[i]));
        }
...
void guardar1(GtkWidget *dummy, gpointer entrada){
    int lista[cantE+1];
    for(int i=0;i<cantE+1;i++){
        lista[i] = gtk_spin_button_get_value (GTK_SPIN_BUTTON(pesos[i]));
    }
    char nombre[255];
...
void ventanaIngresarValores(){
    ...
    GtkWidget *lblNodos = gtk_label_new("Matrices:");
	GdkColor color;
    gdk_color_parse("#FFFFFF", &color);
    gtk_widget_modify_fg(lblNodos, GTK_STATE_NORMAL, &color);
